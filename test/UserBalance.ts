...
describe("UserBalance.sol", function () {
  ...
  describe("ETHbalance", function () {
    ...
    it("Should change ETH balance", async function () {
      ...
      expect(await sut.connect(initAccount).getEthBalance(SOME_OTHER_ADDRESS)).to.equal(0);
      expect(await sut.connect(initAccount).changeEthBalance(SOME_OTHER_ADDRESS, SOME_POSITIVE_AMOUNT)).not.to.be.reverted;
      expect(await sut.connect(initAccount).getEthBalance(SOME_OTHER_ADDRESS)).to.equal(SOME_POSITIVE_AMOUNT);
    });
  ...
  describe("ERC20balance", function () {
    it("Should not allow unauthorised address to change ERC20 balance", async function () {
      ...
      await expect(sut.connect(ownerAccount).changeERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS, SOME_POSITIVE_AMOUNT)).to.be.revertedWith(
          'Caller address is not whitelisted'
      );
    });
    ...
    it("Should change ERC20 balance", async function () {
      ...
      expect(await sut.connect(initAccount).getERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS)).to.equal(0);
      expect(await sut.connect(initAccount).changeERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS, SOME_POSITIVE_AMOUNT)).not.to.be.reverted;
      expect(await sut.connect(initAccount).getERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS)).to.equal(SOME_POSITIVE_AMOUNT);
      expect(await sut.connect(initAccount).changeERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS, SOME_SMALLER_NEGATIVE_AMOUNT)).not.to.be.reverted;
      expect(await sut.connect(initAccount).getERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS)).to.equal(SOME_POSITIVE_AMOUNT + SOME_SMALLER_NEGATIVE_AMOUNT);


      expect(await sut.connect(initAccount).getERC20balance(USDC_SEPOLIA_ADDRESS, SOME_OTHER_ADDRESS)).to.equal(0);
      expect(await sut.connect(initAccount).changeERC20balance(USDC_SEPOLIA_ADDRESS, SOME_OTHER_ADDRESS, SOME_POSITIVE_AMOUNT)).not.to.be.reverted;
      expect(await sut.connect(initAccount).getERC20balance(USDC_SEPOLIA_ADDRESS, SOME_OTHER_ADDRESS)).to.equal(SOME_POSITIVE_AMOUNT);

      expect(await sut.connect(initAccount).getERC20balance(USDC_DAI_ADDRESS, SOME_ADDRESS)).to.equal(0);
      expect(await sut.connect(initAccount).changeERC20balance(USDC_DAI_ADDRESS, SOME_ADDRESS, SOME_POSITIVE_AMOUNT)).not.to.be.reverted;
      expect(await sut.connect(initAccount).getERC20balance(USDC_DAI_ADDRESS, SOME_ADDRESS)).to.equal(SOME_POSITIVE_AMOUNT);
    });
    ...
    it("Should not decrease ERC20 balance by more than its value", async function () {
      ...
      expect(await sut.connect(initAccount).changeERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS, SOME_POSITIVE_AMOUNT)).not.to.be.reverted;
      await expect(sut.connect(initAccount).changeERC20balance(USDC_SEPOLIA_ADDRESS, SOME_ADDRESS, -(SOME_POSITIVE_AMOUNT + 1))).to.be.revertedWith(
          'You are trying to reduce balance by higher amount than it is'
      );
    });
  });
